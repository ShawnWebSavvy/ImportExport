...
use App\Mail\TransactionsCompleteEmail;
...
class processTransactions extends Command
{
     ...
     * @var string
     */
    protected $signature = 'command:processTransactions';
    ...
    public function handle()
    {
        ...
        // Class 'App\Console\Commands\Storage' not found 
        ...
        // build header for Capitec and Nedbank, insert into text file
        ...
        $pathCapitec = 'MercantileCapitec_'.date("Y_m_d").'.txt';
        $file = fopen($pathCapitec,"w");

        $headerCapitec = 'H04Test'.$actionDateTo.$actionDateTo.$actionDateFrom.$actionDateTo.'000001'.$generation_number.'TWO DAY   '.'01';
        fwrite($file, $headerCapitec."\r\n");

        $totalCapitec = count($_SESSION['capitecArray']) +1;
        ...
        foreach ($_SESSION['capitecArray'] as $row) {
            ...
            fwrite($file, $std_transaction_record."\r\n");
        }
        ...
        $capitecDebit = substr($capitecDebit, $str_length, 12);
        // debit and credit transaction  total count must be 6 digits
        $str_length = strlen($totalCredit);
        $zero = '000000000000';
        $capitecCredit = $zero . $totalCredit;
        $capitecCredit = substr($capitecCredit, $str_length, 12);

        // debit and credit transaction  total count must be 6 digits
        $str_length = strlen($debitTransactionsTotalRows);
        $zero = '000000';
        $debitTransactionsTotalRows = $zero . $debitTransactionsTotalRows;
        $debitTransactionsTotalRows = substr($debitTransactionsTotalRows, $str_length, 6);
        // debit and credit transaction  total count must be 6 digits
        $str_length = strlen($creditTransactionsTotalRows);
        $zero = '000000';
        $creditTransactionsTotalRows = $zero . $creditTransactionsTotalRows;
        $creditTransactionsTotalRows = substr($creditTransactionsTotalRows, $str_length, 6);
        ...
        fclose($file);
        File::move($pathCapitec, storage_path('app/downloads/mercantile/current/'.$pathCapitec));

        //$pathNedbank = '../../../storage/app/downloads/mercantile/current/MercantileNedbank_'.date("Y_m_d").'.txt';
        $pathNedbank = 'MercantileNedbank_'.date("Y_m_d").'.txt';
        $file = fopen($pathNedbank,"w");
        
        $headerNedbank = DB::table('mercantile_headers')->first();
        fwrite($file, $headerNedbank->HeaderRow."\r\n");
        ...
        fwrite($file, $trailer."\r\n");
        fclose($file);
        File::move($pathNedbank, storage_path('app/downloads/mercantile/current/'.$pathNedbank));
        ...
        ->orderBy('id','asc')
        ->get();
        // ->chunk(300000, function ($transactions) {
        ...
        DB::table('mercantile_transactions')->delete();
        foreach($transactions as $transaction){
            $RecordIdentifier = substr($transaction->row, 0, 2);
            $AccountHolderFullName = substr($transaction->row, 124, 30);
            $explode = explode(" ", $AccountHolderFullName);
        ...
        // delete deductions table, for a new clean import TransactionsCompleteEmail
        DB::table('deductions')->delete();

        Mail::to('tracyv@leza.co.za')->send(new TransactionsCompleteEmail());
        Mail::to('shawnw@leza.co.za')->send(new TransactionsCompleteEmail());
    }
}
